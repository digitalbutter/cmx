...
function createSetting(&$modx,$key,$value) {
    ...
    if (empty($ct)) {
        ...
        $setting->set('key','cmx.'.$key);
        $setting->set('value',$value);
        $setting->set('namespace','cmx');
...
if ($object->xpdo) {
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            ...
            /* setup paths */
            createSetting($modx,'core_path',$modx->getOption('core_path').'components/cmx/');
            createSetting($modx,'assets_path',$modx->getOption('assets_path').'components/cmx/');

            /* setup urls */
            createSetting($modx,'assets_url',$modx->getOption('assets_url').'components/cmx/');
        break;
    }
}
return true;