 ...
 */
$modx->regClientCSS($cmx->config['cssUrl'].'mgr.css');
$modx->regClientStartupScript($cmx->config['jsUrl'].'mgr/cmx.js');
$modx->regClientStartupHTMLBlock('<script type="text/javascript">
Ext.onReady(function() {
    cmx.config = '.$modx->toJSON($cmx->config).';
    cmx.config.connector_url = "'.$cmx->config['connectorUrl'].'";
    cmx.action = "'.(!empty($_REQUEST['a']) ? $_REQUEST['a'] : 0).'";
});
</script>');
